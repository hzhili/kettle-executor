...
import top.heyx.kettle.exception.ExecutorException;
...
public class FileUtil extends FileUtils {
    ...
    public static void writeFile(byte[] inputByte, File outputFile, boolean overwrite) {
        ...
        if (!overwrite && outputFile.exists()) {
            throw new ExecutorException("文件已存在");
        }
        try {
            // 输出文件不存在就创建
            touch(outputFile);
    ...
    public static String uploadFileRename(MultipartFile file, String savePath) {
        ...
        if (StringUtils.isBlank(originalFilename)) {
            throw new ExecutorException("获取源文件名失败");
        }
        ...
        try {
            ...
            throw new ExecutorException("读取上传文件流失败");
        }
    ...
    public static String replaceSeparator(String str) {
        return StringUtils.isEmpty(str) ? "" : str.replaceAll("\\\\", separator);
    }
